...
The more code is ***made*** available, the more ***shortcuts*** can be realized. Due to the nature of `primitive succeded` I seperate pathes taken into parts belonging to `primitive failed` and all the rest. After `primitive routines` have done their thing (just before they return the result), the size (in bytes) of their `machine instruction` s is accumulated for investigating changes of `instruction path length` and the difference in timing of the test.<br>
