...
The solution in `SmallObjects` is to maintain `private field`s (the `carryall`) after the last data item of the `variaPart` (of the _patron_ instance, e.g. a `ByteString` ;-). And the `class header` bitshave  (plenty of room ;-) the count 0…15 of `private field`s.<br>
